...
def _check_and_send_alerts(server, metric):
    ...
    try:
        ...
        # Check CPU threshold
        if metric.cpu_percent is not None and metric.cpu_percent >= config.cpu_threshold:
            alerts.append({
                'type': 'CPU',
        ...
        if metric.memory_percent is not None and metric.memory_percent >= config.memory_threshold:
            alerts.append({
                'type': 'Memory',
        ...
        if metric.disk_usage:
            try:
                ...
                for mountpoint, usage in disk_data.items():
                    if isinstance(usage, dict) and usage.get('percent', 0) >= config.disk_threshold:
                        alerts.append({
                            'type': 'Disk',
                            'value': usage.get('percent', 0),
                            'threshold': config.disk_threshold,
                            'message': f"Disk usage on {mountpoint} is {usage.get('percent', 0):.1f}% (threshold: {config.disk_threshold}%)"
                        })
                        print(f"[ALERT] Disk threshold exceeded on {mountpoint}: {usage.get('percent', 0):.1f}% >= {config.disk_threshold}%")
...
def edit_admin_user(request, user_id):
    ...
    if request.method == "POST":
        ...
        return redirect("admin_users")
    return render(request, "core/edit_admin_user.html", {"user": user})
