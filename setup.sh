...
# ---- args ------------------------------------------------------------------------
HOST=""; SSL=""; EMAIL=""; CERT=""; KEY=""; DRY_RUN=false; NONINTERACTIVE=false
...
CSRF_TRUSTED_ORIGINS=https://${HOST},http://${HOST}
USE_TLS=${USE_TLS}
...
render_nginx() {  # $1 = "http-only" (LE bootstrap) | "tls"
  ...
  local proxy='        proxy_pass http://web:8000;
        ...
        proxy_redirect off;'
  cat <<EOF
# Generated by setup.sh -- SSL mode: ${SSL}
server {
    listen 80;
    server_name ${HOST};
...
if $DRY_RUN; then
  ...
  info "host=$HOST  ssl=$SSL  email=${EMAIL:-n/a}  USE_TLS=$USE_TLS"
  ...
  info "wait for http://127.0.0.1:8000/health/  ->  print  https://$HOST/setup"
  exit 0
fi
